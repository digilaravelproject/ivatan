...
class AdminSettingController extends Controller
{
    ...
    public function index(): View
    {
        try {
            ...
            Log::error('Settings index load failed', ['error' => $e->getMessage()]);
            abort(500, 'Failed to load settings: ' . $e->getMessage());
        }
    }
    ...
    public function updatePayment(Request $request): RedirectResponse
    {
        try {
            ...
            Log::error('Payment settings update failed', ['error' => $e->getMessage()]);
            return redirect()->back()
                ->withInput()
                ->with('error', 'An error occurred while saving payment settings: ' . $e->getMessage());
        }
    }
    ...
    public function updateSubscription(Request $request): RedirectResponse
    {
        try {
            ...
            Log::error('Subscription settings update failed', ['error' => $e->getMessage()]);
            return redirect()->back()
                ->withInput()
                ->with('error', 'An error occurred while saving subscription settings: ' . $e->getMessage());
        }
    }
    ...
    public function updateGeneral(Request $request): RedirectResponse
    {
        try {
            ...
            Log::error('General settings update failed', ['error' => $e->getMessage()]);
            return redirect()->back()
                ->withInput()
                ->with('error', 'An error occurred while saving general settings: ' . $e->getMessage());
        }
    }
}
