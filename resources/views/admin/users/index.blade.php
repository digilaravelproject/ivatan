                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3 text-sm">
                                    ...
                                    @elseif(isset($user->status) && $user->status === 'inactive')
                                        <span
                                            class="px-2 py-1 text-xs text-yellow-700 bg-yellow-100 rounded">Inactive</span>
                                    @else
                                ...
                                <td class="px-4 py-3 text-sm text-right">
                                    <div class="flex flex-wrap justify-end gap-2">
                                        <a href="{{ route('admin.users.show', $user) }}"
                                            class="px-3 py-1 text-sm text-blue-700 rounded bg-blue-50">View</a>
                                        ...
                                        @if (!$user->is_seller)
                                            <form action="{{ route('admin.users.seller.toggle', $user) }}"
                                                method="POST">
                                                @csrf @method('PUT')
                                                <button type="submit"
                                                    onclick="return confirm('Enable seller status for this user?')"
                                                    class="px-3 py-1 text-sm text-indigo-700 rounded bg-indigo-50">Enable
                                                    Seller</button>
                                            </form>
                                        @else
                                            <form action="{{ route('admin.users.seller.toggle', $user) }}"
                                                ...
                                                <button type="submit"
                                                    ...
                                                    class="px-3 py-1 text-sm text-red-700 rounded bg-red-50">Disable
                                                    Seller</button>
                                            </form>
                                        @endif

                                        @if (!$user->is_employer)
                                            <form action="{{ route('admin.users.employer.toggle', $user) }}"
                                                method="POST">
                                                @csrf @method('PUT')
                                                <button type="submit"
